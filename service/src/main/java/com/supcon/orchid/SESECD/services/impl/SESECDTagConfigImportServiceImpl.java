...
@Service("sESECDTagConfigImportService")
public class SESECDTagConfigImportServiceImpl implements SESECDTagConfigImportService {
	...
	@Override
	@AuditLog(entity="SESECD_1.0.0_tagConfig",model="SESECD_1.0.0_tagConfig_TagConfig",desc="保存业务信息",operType="1")
	public void saveImportDataLog(SESECDTagConfig bizObj, SESECDTagConfig oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("SESECD.entityname.randon1616138863441") + "信息");
			...
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == oldBizObj.getTagName() ? "" : "," + oldBizObj.getTagName().toString()));
			} else {
				AuditUtil.setColumnStringB(null == oldBizObj.getTagName() ? "" : oldBizObj.getTagName().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("SESECD.entityname.randon1616138863441") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
			...
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == bizObj.getTagName() ? "" : "," + bizObj.getTagName().toString()));
			} else {
				AuditUtil.setColumnStringB(null == bizObj.getTagName() ? "" : bizObj.getTagName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
	...
	private void setBussinessKeyAndMainDispalyField(SESECDTagConfig bizObj, String bussinesskeyFieldName,
