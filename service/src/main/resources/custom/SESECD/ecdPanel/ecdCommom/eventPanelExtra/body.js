    	...
    	ReactAPI.request(
           ...
           function(res) {
              if(res.code == 200){
                  ...
                  if(data){
                    debugger
                      var grid = ReactAPI.getComponentAPI("SupDataGrid").APIs("SESECD_1.0.0_ecdPanel_eventPanelExtradg1578378703400");
                      var Arry = new Array(); 
    ...
    if(eventId != undefined && eventId != "" &&  eventId !=null){
    	...
    	ReactAPI.request(
           ...
           function(res) {
              if(res.code == 200){
                  var data = res.data.result;
                  var grid = ReactAPI.getComponentAPI("SupDataGrid").APIs("SESECD_1.0.0_ecdPanel_eventPanelExtradg1578378709509");
                  if(data.length==0){
				      return;
				  }
                  var Arry = new Array(); 
...
function updateStiuation(){
    ...
    var code = 'SESECD_1.0.0_emcSituation_situationList_self'
    var __pc__ = ReactAPI.getPowerCode(code);
    var url = "/msService/SESECD/emcSituation/emcSituation/situationEdit?__pc__="+__pc__[code]+"&id="+rows[0].stateId.id + "&viewCode=SESECD_1.0.0_emcSituation_situationMapView&entityCode=SESECD_1.0.0_emcSituation&openType=frame&viewType=edit";
...
function actionEdit(){
    ...
    var code = 'SESECD_1.0.0_emcAction_actionList_self'
    var __pc__ = ReactAPI.getPowerCode(code);
    var url = "/msService/SESECD/emcAction/emcAction/actionUpdate?__pc__="+__pc__[code]+"&id="+rows[0].actionId.id + "&viewCode=SESECD_1.0.0_emcAction_actionMapView&entityCode=SESECD_1.0.0_emcAction&openType=frame&viewType=edit";
	window.open(url,"_blank");
}
...
function setIntrocutClickEvent(){
	ReactAPI.getComponentAPI("SupDataGrid")
	  .APIs("SESECD_1.0.0_ecdPanel_eventPanelExtradg1578378703400")
	  .setClickEvt(function(e, data) {
        console.log(data)
        var msg = {
          data:{
              instructionId:data.commomId.id
              //instructionId:104997371544832
          },
          type:"instruction"
		}
...
function setSituationClickEvent(){
	ReactAPI.getComponentAPI("SupDataGrid")
	  .APIs("SESECD_1.0.0_ecdPanel_eventPanelExtradg1578378709509")
	  .setClickEvt(function(e, data) {
        var msg = {
          data:{
...
function setActionClickEvent(){
	ReactAPI.getComponentAPI("SupDataGrid")
	  .APIs("SESECD_1.0.0_ecdPanel_eventPanelExtradg1578378713744")
	  .setClickEvt(function(e, data) {
        var msg = {
          data:{
