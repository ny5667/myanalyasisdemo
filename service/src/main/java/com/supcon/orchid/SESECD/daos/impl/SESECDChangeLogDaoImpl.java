...
/* CUSTOM CODE START(daoimpl,import,SESECD_1.0.0_changeLog_ChangeLog,SESECD_1.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Repository
public class SESECDChangeLogDaoImpl extends ExtendedGenericDaoImpl<SESECDChangeLog, Long> implements SESECDChangeLogDao {
	@Override
	...
	@Override
	public SESECDChangeLogDealInfo copyAndSaveDealInfo(DealInfo di) {
		SESECDChangeLogDealInfo dealInfo = new SESECDChangeLogDealInfo();
	...
	@Override
	public SESECDChangeLogDealInfo copyAndSaveDealInfo(DealInfo di, SESECDChangeLog mainObj, Staff dealStaff) {
		SESECDChangeLogDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(SESECDChangeLogDealInfo.class);
		...
		List<SESECDChangeLogDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,SESECD_1.0.0_changeLog_ChangeLog,SESECD_1.0.0) */
// 自定义代码
