...
function customDelete() {
  ...
  var actionRows = ReactAPI.getComponentAPI("SupDataGrid")
    .APIs("SESECD_1.0.0_alarmRecord_alarmRecordEditdg1576762078782")
    .getDatagridData();
  ...
  ReactAPI.getComponentAPI("SupDataGrid")
    .APIs("SESECD_1.0.0_alarmRecord_alarmRecordEditdg1576762038198")
    .deleteLine(planRowIndex);
...
function onPlanChange() {
  ...
  var planRows = ReactAPI.getComponentAPI("SupDataGrid")
    .APIs("SESECD_1.0.0_alarmRecord_alarmRecordEditdg1576762038198")
    .getDatagridData();
  ...
  if (actionDeleteRowIndexArray.length > 0) {
    ReactAPI.getComponentAPI("SupDataGrid")
      .APIs("SESECD_1.0.0_alarmRecord_alarmRecordEditdg1576762078782")
      .deleteLine(actionDeleteRowIndexArray.join(','));
  }
...
function saveActionGridData() {
  ...
  if (actionRowArray.length > 0) {
    ReactAPI.getComponentAPI("SupDataGrid")
      .APIs("SESECD_1.0.0_alarmRecord_alarmRecordEditdg1576762078782")
      .saveAddLine(actionRowArray.join(','));
  }
...
//事故类型删除触发事件
function alarmIdEnenetrelAfterClear(deleteObj){debugger
    let typeArr = [];
...
//事故类型添加出发事件
function alarmIdEnenetrelOnAfterSetPosition(MneObj){debugger
   var planArr = []
...
let typeArr = [];
for(let i = 0;i<MneObj.length;i++){
     typeArr.push(MneObj[i].id);
}
console.log(arr);
  //ReactAPI.getComponentAPI().SystemCode.APIs("alarmRecord.alarmLevel").getValue()  {value: 'VxECD_alarmLevel/004', label: '四级'}
  //报警等级
  var alarmLevel;
  if ( ReactAPI.getComponentAPI().SystemCode.APIs("almAlarmRecord.alarmLevel").getValue().length !== 0){
    alarmLevel = ReactAPI.getComponentAPI().SystemCode.APIs("almAlarmRecord.alarmLevel").getValue().value;
  }
ReactAPI.request(
  {
    type: "post",
...
//报警等级onchange事件
function changeAlarmLevel(val){debugger
  //1 已选事故类型id集合
  var typeArr = [];
  //当前选中事故类型
  var arr = ReactAPI.getComponentAPI().Reference.APIs("almAlarmRecord.alarmIdEnenetrel").getValue();
  for(var i = 0 ; i<arr.length ;i++){
    typeArr.push(arr[i].id)
  }
  //2 已有预案值，可能是用户手动添加
  var planArr = []
  var arr =  ReactAPI.getComponentAPI().Reference.APIs("almAlarmRecord.alarmIdPlanObj").getValue();
