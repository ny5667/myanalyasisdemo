...
/* CUSTOM CODE START(daoimpl,import,SESECD_1.0.0_situationSentence_Sentence,SESECD_1.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Repository
public class SESECDSentenceDaoImpl extends ExtendedGenericDaoImpl<SESECDSentence, Long> implements SESECDSentenceDao {
	@Override
	...
	@Override
	public SESECDSentenceDealInfo copyAndSaveDealInfo(DealInfo di) {
		SESECDSentenceDealInfo dealInfo = new SESECDSentenceDealInfo();
	...
	@Override
	public SESECDSentenceDealInfo copyAndSaveDealInfo(DealInfo di, SESECDSentence mainObj, Staff dealStaff) {
		SESECDSentenceDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(SESECDSentenceDealInfo.class);
		...
		List<SESECDSentenceDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,SESECD_1.0.0_situationSentence_Sentence,SESECD_1.0.0) */
// 自定义代码
