...
function alarmCheck(){
    ...
    ReactAPI.createDialog("checkAlarmDialog", {
        ...
        buttons: [{
            ...
            onClick: function(event) { //批量处警，将处理状态和处理记录回填到相应的接警中
                ...
                var responseLevel = event.ReactAPI.getComponentAPI("SystemCode").APIs("almAlarmRecord.responseLevel").getValue().value
                //事件分类
                var evtype = event.ReactAPI.getComponentAPI("SystemCode").APIs("almAlarmRecord.evTypes").getValue().value
                ...
                ReactAPI.request({
                        ...
                        data: {
                            //接警IDS
                            alarmId: grid.getSelecteds()[0].id,
                            responseLevel:responseLevel,
                            evType:evtype
                        },
                        url: "/msService/SESECD/alarmRecord/alarmRecord/alarmCheck",
                    },
                    function(res) {
                        console.log(res);
                        if (res.code === 200) {
                            ...
                            ReactAPI.showMessage("s", ReactAPI.international.getText("SESECD.custom.random1692261352467"), '', false);
                            setTimeout(function() {
                                    ReactAPI.getComponentAPI("SupDataGrid").APIs("SESECD_1.0.0_alarmRecord_alarmHandleList_almAlarmRecord_sdg").refreshDataByRequst({
                                        type: "post",
                                        url: "/msService/SESECD/alarmRecord/almAlarmRecord/alarmHandleList-query",
                            ...
                            ReactAPI.destroyDialog("checkAlarmDialog");
                        }
                    });
