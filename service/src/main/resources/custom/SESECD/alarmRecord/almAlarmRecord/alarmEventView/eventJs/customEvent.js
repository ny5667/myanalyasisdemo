...
document.getElementById("process-btn-con").style.display = "none";
// 临时处理地图未铺满
$('head').append('<style>.ant-spin-nested-loading{height:100%}.ant-spin-container{height:100%;}</style>');
//临时处理不显示点
const interval = setInterval(()=>{if(document.getElementsByTagName("iframe")&&document.getElementsByTagName("iframe")[0]){document.getElementsByTagName("iframe")[0].src = '/msService/SESGISConfig/themeConfig/sesGisApi/index.html?layerCode=incidentLayer&mode=look&type=point&spatialId=SESECD_1.0.0_alarmRecord_AlmAlarmRecord_'+getUrlParams().id};clearInterval(interval)},200)
}
